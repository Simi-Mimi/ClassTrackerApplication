...
## שימוש במערכת
הכניסה למערכת מתבצעת דרך עמוד הבית, בו ניתן לבחור את סוג המשתמש (מורה או מנהל). כל כניסה דורשת הזנת מספר זהות לאימות.
...
<img width="1365" height="601" alt="image" src="https://github.com/user-attachments/assets/32cb4dec-9cc8-4022-99ae-71b248355a11" />
...
# ממשק מורה
מכיל צפיה ברשימת התלמידים של המורה שנכנס כעת למערכת, וכן צפיה במפה המסמנת את מיקומי התלמידים- מורה בירוק, תלמידים בכחול ותלמידים שהתרחקו מסומנים באדום,
...
## הנחות מקלות
1. אין מחיקה ועדכון רשומות
2. בשביל לדמות כיצד המערכת עובדת, יצרתי קובץ סימולציה נפרד המדמה את מכשיר האיכון
3. הנחתי שהמערכת מיועדת עבור הנהלת ביה"ס ולכן מספיק אבטחה בסיסית של זיהוי על פי מספר זהות (ולא JWT- סיסמאות, כתובות מייל וכדו')
4. הנחתי שה-ID שהמכשיר שולח הינו מס זהות (ולא ID של המכשיר בעצמו)

## הצעות לשיפור
1. יש דרישה שהגישה ל-API תהיה חשופה למורים בלבד, ולכן הורים שיבקשו לדעת היכן ילדיהם- ישלח להם המורה את נתוני הילד שלהם ביחס למיקום המורה.
2. ניקיתי אוטומטית מיקומים ישנים שנשלחו ממכשיר האיכון, אך עדיף לשמור את הנתונים עבור הוספת פיצ'רים עתידיים (כגון סימון מסלול וכו')
3. הוספתי טבלה ב-DB עבור מכשיר האיכון עצמו, וכן קישרתי אותו לישויות המתאימות עבור שימוש עתידי 
# להלן מסמך ERD שמפרט את הטבלאות שיצרתי
<img width="319" height="449" alt="image" src="https://github.com/user-attachments/assets/c1c88518-dc7e-475f-9360-2aa220b4c2af" />
